...
def simulate_kuramoto(N, K, noise_level, steps=500, dt=0.05): theta = np.random.uniform(0, 2 * np.pi, N) omega = np.random.normal(1.0, 0.1, N) for _ in range(steps): interaction = np.sum(np.sin(np.subtract.outer(theta, theta)), axis=1) noise = np.random.normal(0, noise_level, N) theta += (omega + (K / N) * interaction + noise) * dt return theta

📏 Rendparaméter

def order_parameter(theta): return np.abs(np.mean(np.exp(1j * theta)))

🚀 Streamlit app

def run(): st.set_page_config(layout="wide") st.title("📉 Zajtűrés és szinkronizáció robusztusság")
