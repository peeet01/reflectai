...
class Generator(nn.Module): def init(self, z_dim=64, img_dim=28 * 28): super().init() self.net = nn.Sequential( nn.Linear(z_dim, 256), nn.ReLU(True), nn.Linear(256, 512), nn.ReLU(True), nn.Linear(512, img_dim), nn.Tanh() )
...
def run(): st.set_page_config(layout="wide") st.title("🧪 GAN – Generative Adversarial Network")
