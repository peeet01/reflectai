import streamlit as st import numpy as np import torch import torch.nn as nn import time import pandas as pd import matplotlib.pyplot as plt import seaborn as sns from sklearn.metrics import confusion_matrix from mpl_toolkits.mplot3d import Axes3D  # 3D vizualizációhoz import plotly.graph_objects as go  # Interaktív 3D-hez

🔧 Moduláris modell definiálása

class XORNet(nn.Module): def init(self, input_size=2, hidden_size=4, activation=nn.Tanh): super(XORNet, self).init() self.fc1 = nn.Linear(input_size, hidden_size) self.act1 = activation() self.fc2 = nn.Linear(hidden_size, 1) self.act2 = nn.Sigmoid()
...
def add_noise(data, noise_level): noise = noise_level * np.random.randn(*data.shape) return data + noise

💾 Modell mentése

def save_model(model, path="xor_model.pth"): torch.save(model.state_dict(), path)

📊 Pontosság kiértékelése

def evaluate(model, inputs, targets): with torch.no_grad(): predictions = model(inputs).round() accuracy = (predictions.eq(targets).sum().item()) / targets.size(0) return accuracy, predictions

🚀 Fő Streamlit modul

ACTIVATIONS = { "Tanh": nn.Tanh, "ReLU": nn.ReLU, "Sigmoid": nn.Sigmoid, "LeakyReLU": nn.LeakyReLU }

def run(hidden_size=4, learning_rate=0.1, epochs=1000, note=""): st.subheader("🔁 XOR predikció neurális hálóval") st.markdown("Ez a modul egy egyszerű neurális háló segítségével tanítja meg az XOR függvényt, zajjal, mentéssel, exporttal és 3D vizualizációval kiegészítve.")
...
activation_name = st.selectbox("🔌 Aktivációs függvény", list(ACTIVATIONS.keys()), index=0)
...
model = XORNet(hidden_size=hidden_size, activation=activation_fn).to(device)
...
# 🌊 Interaktív 3D hullámszerű predikciós felület
