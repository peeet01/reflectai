import streamlit as st import numpy as np import matplotlib.pyplot as plt import plotly.graph_objects as go import time import os from datetime import datetime
...
def plot_brain_activity(activation_map, grid_size): fig, ax = plt.subplots(figsize=(5, 5)) im = ax.imshow(activation_map, cmap="plasma", interpolation='nearest') ax.set_title("🧠 Aktivációs térkép (neurális mintázat)") ax.set_xlabel("Neuron X") ax.set_ylabel("Neuron Y") fig.colorbar(im, ax=ax, label="Aktiváció gyakoriság") return fig

def plot_brain_activity_3d(activation_map): x = np.arange(activation_map.shape[1]) y = np.arange(activation_map.shape[0]) xx, yy = np.meshgrid(x, y) zz = activation_map

fig = go.Figure(data=[go.Surface(z=zz, x=xx, y=yy, colorscale='Plasma')])
fig.update_layout(
    title='🧠 Aktivációs térkép – 3D agymodell',
    scene=dict(
        xaxis_title='Neuron X',
        yaxis_title='Neuron Y',
        zaxis_title='Aktiváció gyakoriság',
...
tab2d, tab3d = st.tabs(["🖼 2D hőtérkép", "🌐 3D agymodell"])
with tab2d:
    fig_brain = plot_brain_activity(activations[selected_ep], grid_size)
    st.pyplot(fig_brain)
with tab3d:
    fig_3d = plot_brain_activity_3d(activations[selected_ep])
