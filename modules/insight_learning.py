...
📉 Átlagos lépésszám: {np.mean(steps_to_goal):.2f} """ filename = "insight_learning_report.txt" with open(filename, "w", encoding="utf-8") as f: f.write(report_text) with open(filename, "rb") as f: st.download_button("⬇️ Letöltés", f, file_name=filename) os.remove(filename)

with st.expander("📘 Tudományos háttér – Mi az a belátás?"): st.markdown(""" A belátásos tanulás (insight learning) egy kognitív folyamat, ahol a probléma megoldása nem véletlenszerű próbálkozással, hanem egy strukturális átlátás révén történik.
...
A szimulált aktivációs térkép azt reprezentálja, hogy az „agy” mely régiói (pozíciói) milyen gyakran voltak aktívak a sikeres vagy sikertelen keresés során. Amikor az aktiváció meghalad egy kritikus küszöbszintet, az ügynök felismeri a megoldást – ezt vizualizáljuk egy "kitörésként" a domborzati agymodellben. """)
