import streamlit as st import numpy as np import matplotlib.pyplot as plt import time import os from datetime import datetime import plotly.graph_objects as go from scipy.ndimage import gaussian_filter from numpy.random import default_rng
...
def enhance_activation_map(activation_map, scale=0.5): rng = default_rng() noise = rng.normal(0, 1, activation_map.shape) enhanced = activation_map + scale * gaussian_filter(noise, sigma=1.5) return np.clip(enhanced, 0, None)

def plot_brain_activity_3d(activation_map): x, y = np.meshgrid(np.arange(activation_map.shape[1]), np.arange(activation_map.shape[0])) base = gaussian_filter(activation_map, sigma=1.2) z = enhance_activation_map(base, scale=0.7)
...
fig = go.Figure(data=[
    go.Surface(
        ...
        colorscale='Electric',
        showscale=True,
        lighting=dict(ambient=0.4, diffuse=0.9, specular=1.5, roughness=0.2),
        lightposition=dict(x=100, y=200, z=300),
        contours=dict(z=dict(show=True, usecolormap=True, highlightcolor="lime", project_z=True))
    )
])
fig.update_layout(
    title="🧠 Plasztikus 3D agymodell – Aktivációs domborzat",
    scene=dict(
        xaxis_title='Neuron X',
        yaxis_title='Neuron Y',
        zaxis_title='Aktiváció (intenzitás)'
    ),
...
with st.expander("📘 Tudományos háttér – Mi az a belátás?"):
    ...
    A 3D vizualizáció egy **perlin-zajjal javított agydomborzatot** mutat, amely a tanulás intenzitását plasztikusan és organikusan jeleníti meg.
    """)
