...
# Hebbian súlydinamika vizualizáció
st.header("📉 Hebbian tanulás – súlydinamika")
if st.button("Súlydinamika szimuláció"):
    fig_w = run_hebbian_learning_with_visual(
...
if st.button("Ridge predikció futtatása"):
    ...
    ax2.plot(*pred_states.T, label='Predikált (Ridge)', color='orange')
    ax2.set_title("Predikált Lorenz pálya (Ridge)")
    st.pyplot(fig)

# Lorenz MLP
st.header("🧠 Lorenz-attraktor predikció (MLP)")
if st.button("MLP predikció futtatása"):
    t, true_states = generate_lorenz_data()
    ...
    ax2 = fig.add_subplot(122, projection='3d')
    ax2.plot(*pred_states.T, label='Predikált (MLP)', color='green')
    ax2.set_title("Predikált Lorenz pálya (MLP)")
    st.pyplot(fig)