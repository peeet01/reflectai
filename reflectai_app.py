...
st.text_input("💬 Írd be kérdésed vagy megfigyelésed (későbbi nyelvi modulhoz):")
...
module_name = st.sidebar.radio("Kérlek válassz:", (
    ...
    "Generatív Kuramoto szimuláció (GAN)"
))
...
elif module_name == "Generatív Kuramoto szimuláció (GAN)":
    epochs = st.slider("Tanulási ciklusok száma", 100, 5000, 1000, step=100)
    noise_dim = st.slider("Zaj dimenzió", 5, 50, 10)
    num_oscillators = st.slider("Oszcillátorok száma", 3, 20, 10)
    run_generative_kuramoto(epochs, noise_dim, num_oscillators)
